...
from bocchi.services.memory_governor import (
    start_memory_governor,
    stop_memory_governor,
)
from bocchi.services.send_queue import start_send_queue, stop_send_queue
from bocchi.services.uninfo_patch import apply_uninfo_onebot11_patch
...
def _apply_alconna_conflict_patch() -> None:
    ...
    with contextlib.suppress(Exception):
        ...
        if getattr(original_remove, "__bocchi_safe_remove__", False):
        ...
        setattr(_safe_remove, "__bocchi_safe_remove__", True)
...
def _start_launcher_watchdog() -> None:
    ...
    if _launcher_watchdog_task is not None and not _launcher_watchdog_task.done():
        return
    launcher_pid_text = os.getenv("bocchi_LAUNCHER_PID", "").strip()
    ...
    async def _setup_runtime_concurrency() -> None:
        ...
        if _thread_executor is None:
            _thread_executor = ThreadPoolExecutor(
                max_workers=workers, thread_name_prefix="bocchi-worker"
            )
