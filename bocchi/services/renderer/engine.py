...
_LOOP_EXCEPTION_FILTER_STATE_ATTR = "_bocchi_playwright_exception_filter_state"
...
def _patch_htmlrender_task_tracking() -> None:
    if getattr(htmlrender_browser, "_bocchi_task_tracking_patched", False):
        return
    ...
    setattr(htmlrender_browser, "_bocchi_task_tracking_patched", True)


def _patch_htmlrender_shutdown() -> None:
    if getattr(htmlrender_browser, "_bocchi_shutdown_patched", False):
        return
    ...
    setattr(htmlrender_browser, "_bocchi_shutdown_patched", True)


def _patch_playwright_env_check_once() -> None:
    _patch_htmlrender_task_tracking()
    _patch_htmlrender_shutdown()
    if getattr(htmlrender_browser, "_bocchi_check_once_patched", False):
        return
    ...
    if original_check is None:
        ...
        setattr(htmlrender_browser, "_bocchi_check_once_patched", True)
        return
...
class PlaywrightEngine(BaseScreenshotEngine):
    ...
    async def _on_render_begin(self) -> None:
        await _HTMLRENDER_TASK_TRACKER.begin("bocchi_renderer")
        async with self._state_lock:
