...
def apply_uninfo_onebot11_patch() -> None:
    ...
    with contextlib.suppress(Exception):
        ...
        if not getattr(original_endpoint, "__bocchi_fast_onebot11__", False):
            _ORIGINAL_ONEBOT11_GROUP_MESSAGE = cast(
                Callable[..., Awaitable[dict[str, Any]]] | None,
                original_endpoint,
            )
            setattr(_fast_onebot11_group_message, "__bocchi_fast_onebot11__", True)
    ...
    with contextlib.suppress(Exception):
        ...
        if not getattr(original_c2c, "__bocchi_fast_qq__", False):
            _ORIGINAL_QQ_C2C_MESSAGE = cast(
                Callable[..., Awaitable[dict[str, Any]]] | None,
                original_c2c,
            )
            setattr(_fast_qq_c2c_message, "__bocchi_fast_qq__", True)
        ...
        for event_type in (GroupMessageCreateEvent, GroupAtMessageCreateEvent):
            ...
            if getattr(original_group_at, "__bocchi_fast_qq__", False):
                continue
            ...
            setattr(_fast_qq_group_at_message, "__bocchi_fast_qq__", True)
        ...
        for event_type in (
            ...
            if getattr(original_guild, "__bocchi_fast_qq__", False):
                continue
            ...
            setattr(_fast_qq_guild_message, "__bocchi_fast_qq__", True)
    ...
    if getattr(original_fetch, "__bocchi_singleflight__", False):
        _PATCHED = True
        return
    ...
    setattr(_singleflight_fetch, "__bocchi_singleflight__", True)
