...
from bocchi.services.ai.core.messages import AgentMessage, LLMMessage
from bocchi.services.ai.core.stream_events import ToolStreamChunkEvent
from bocchi.services.ai.core.templates import PromptTemplate
from bocchi.services.ai.flow.agent.agent import Agent, ToolSource
from bocchi.services.ai.flow.agent.models import AgentConfig
from bocchi.services.ai.flow.team.models import (
    CallAction,
...
from bocchi.services.ai.run import AgentTask, RunContext
from bocchi.services.ai.tools.bridges.delegate import DelegateTool
from bocchi.services.log import logger

if TYPE_CHECKING:
    from bocchi.services.ai.flow.team.team import Team
...
class RouteStrategy(BaseTeamStrategy):
    ...
    def __init__(
        ...
        if isinstance(state_flow, dict):
            from bocchi.services.ai.flow.team.models import Transition
...
请根据结果决定是下发新任务、要求重做，还是调用 `mark_all_complete` 汇报总结。
    ...
    def __init__(
        ...
        if blackboard_schema is not None:
            from bocchi.services.ai.run.blackboard import BlackboardManager
            from bocchi.services.ai.tools.providers.builtin.blackboard import (
                BlackboardToolkit,
            )
    ...
    async def generate_plan(
        ...
        **kwargs,
        ...
        from bocchi.services.ai.flow.team.models import TaskBoardState, TaskNodeStatus
        from bocchi.services.ai.flow.team.task_tools import TaskPlanningToolkit
