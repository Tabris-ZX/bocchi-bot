...
from bocchi.services.ai.context.memory.storage.backends import (
    InMemoryChatContext,
    MemoryScope,
)
from bocchi.services.ai.context.memory.storage.interfaces import (
    BaseChatContext,
    BaseSlotContext,
)
from bocchi.services.ai.context.rag.backends import Embedder, StorageBackend
from bocchi.services.ai.utils.scope import BaseScopeBuilder
...
class MemoryCleaner(BaseScopeBuilder["MemoryCleaner"]):
    ...
    async def clear_long_term(self):
        ...
        if self._config and self._config.long_term.backend:
            from bocchi.services.ai.context.rag.backends import StorageBackend
...
class GlobalMemoryManager:
    ...
    def get_embedder(self, embedder_val: "Embedder | str | None") -> Embedder | None:
        ...
        if isinstance(embedder_val, str):
            from bocchi.services.ai.context.rag.backends.embedders import (
                DefaultEmbedder,
            )
    ...
    def get_long_term_memory(
        ...
        from bocchi.services.ai.context.memory.models import MemoryScoringConfig
