...
from bocchi.services.ai.context.rag.retrieval import FilterEvaluator
...
class QdrantStorageBackend(StorageBackend):
    ...
    def __init__(
        self,
        location: str = ":memory:",
        collection_name: str = "bocchi_rag",
        **kwargs,
    ):
        try:
...
class LanceDBStorageBackend(StorageBackend):
    ...
    def __init__(
        self, uri: str = "./data/lancedb", table_name: str = "bocchi_rag", **kwargs
    ):
        try:
            import lancedb
        except ImportError:
