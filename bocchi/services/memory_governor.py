...
from bocchi.services.message_load import idle_seconds, is_overloaded
...
async def _has_active_auth_work() -> bool:
    module = sys.modules.get("bocchi.builtin_plugins.hooks.auth_checker")
    if module is None:
        return False
...
async def _has_active_render_work() -> bool:
    module = sys.modules.get("bocchi.services.renderer.engine")
    if module is None:
        return False
...
async def _flush_statistics_buffer() -> int:
    module = sys.modules.get("bocchi.builtin_plugins.statistics.statistics_hook")
    if module is None:
        return 0
...
async def _flush_user_gold_log_buffer() -> int:
    module = sys.modules.get("bocchi.services.buffered_writers")
    if module is None:
        return 0
...
def _clear_runtime_negative_caches() -> dict[str, int]:
    module = sys.modules.get("bocchi.services.cache.runtime_cache")
    if module is None:
        return {}
    result: dict[str, int] = {}
    for name in (
...
def _clear_avatar_memory_cache() -> int:
    module = sys.modules.get("bocchi.services.avatar_service")
    if module is None:
        return 0
...
async def _clear_renderer_runtime_caches() -> dict[str, int]:
    module = sys.modules.get("bocchi.services.renderer.service")
    if module is None:
        return {}
...
def _clear_message_manager_cache() -> int:
    module = sys.modules.get("bocchi.utils.manager.message_manager")
    if module is None:
        return 0
