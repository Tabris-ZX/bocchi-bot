...
def _print_version() -> None:
    try:
        ver = importlib.metadata.version("bocchi-bot")
    except importlib.metadata.PackageNotFoundError:
        ver = "unknown"
    sys.stdout.write(f"bocchi-bot {ver}\n")


def _ensure_project_root() -> Path:
    cwd = Path.cwd()
    if not (cwd / "bocchi").is_dir():
        sys.stderr.write("错误: 当前目录不是 bocchi_bot 项目目录。\n")
        sys.stderr.write("请在项目根目录（包含 bocchi/ 目录的位置）执行 zx run。\n")
        sys.exit(1)
...
def _build_worker_command() -> list[str]:
    return [sys.executable, "-m", "bocchi.cli", "run-worker"]
...
def _run_launcher() -> None:
    ...
    from bocchi.utils.restart_state import (
    ...
    while True:
        ...
        worker_env["bocchi_LAUNCHER_PID"] = str(os.getpid())
