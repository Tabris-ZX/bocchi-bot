...
from bocchi.services.db_context.schema_ops import AlterColumnType, DropColumn
...
class GroupInfoUser(Model):
    ...
    async def get_all_uid(cls, group_id: str) -> set[str]:
        ...
        参数:
            group_id: 群号
        """
        from bocchi.services.hot_query_cache import get_group_user_ids
    ...
    async def get_user_all_group(cls, user_id: str) -> list[str]:
        ...
        参数:
            user_id: 用户id
        """
        from bocchi.services.hot_query_cache import get_user_group_ids
