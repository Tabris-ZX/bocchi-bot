...
class BaseAnalyzer(ABC, Generic[TDataObject, TInputData]):
    ...
    def _save_debug_data(self, prompt: str, session_id: str):
        ...
        try:
            ...
            data_path = DATA_PATH / PLUGIN_NAME / "debug_data"
    ...
    async def _build_system_prompt(self, umo: str | None) -> str | None:
        ...
        # NoneBot/Bocchi currently has no session-persona API compatible with the
        # upstream implementation. A host-provided persona manager may still be
        # used for an explicitly configured fixed persona.
