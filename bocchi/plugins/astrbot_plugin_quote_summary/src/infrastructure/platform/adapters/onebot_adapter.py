...
class OneBotAdapter(PlatformAdapter):
    ...
    async def _detect_llbot(self):
        ...
        try:
            ...
            result = await self.bot.call_api("get_version_info")
    ...
    async def fetch_messages(
        ...
        """
        if not hasattr(self.bot, "call_api"):
            return []
        ...
        try:
            ...
            while len(all_raw_messages) < max_count:
                ...
                result = await self.bot.call_api("get_group_msg_history", **params)
    ...
    async def get_group_list(self) -> list[str]:
        ...
        try:
            result = await self.bot.call_api("get_group_list")
        ...
        if not bot_user_id:
            try:
                ...
                login_info = await asyncio.wait_for(
                    self.bot.call_api("get_login_info"), timeout=5.0
                )
    ...
    async def upload_group_file_to_folder(
        ...
        async def do_upload(content: str, label: str):
            ...
            await self.bot.call_api("upload_group_file", **params)
    ...
    async def upload_group_album(
        ...
        async def do_upload(content: str, label: str):
            ...
            if self._is_llbot:
                ...
                try:
                    await self.bot.call_api("upload_group_album", **llbot_params)
                    logger.debug(
            ...
            for action in [
                ...
                try:
                    await self.bot.call_api(action, **params)
                    logger.debug(
