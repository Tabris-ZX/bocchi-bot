...
from bocchi.services.ai.context.memory.storage import AbstractMemoryRecord
from bocchi.services.ai.context.rag import AbstractVectorRecord
from bocchi.services.ai.core.messages import LLMMessage


class BocchiGroupConfig(BaseModel):
    """bocchi_ai 专属的群组级动态配置模型"""

    current_persona: str = Field(default="波奇", description="当前群使用的 AI 人设名称")
...
class BocchiAiMemoryRecord(AbstractMemoryRecord):
    class Meta:  # type: ignore
        table = "bocchi_ai_memory"
        table_description = "Bym聊天记忆表"


class BocchiAiVectorRecord(AbstractVectorRecord):
    class Meta:  # type: ignore
        table = "bocchi_ai_vector_memory"
...
class BocchiMemorySettings(BaseModel):
    """插件全局 Memory 总配置根节点"""
