...
from bocchi.services.ai.context.memory import memory_manager
...
async def _(arp: Arparma):
    ...
    if resp is None:
        await bocchi_cmd.finish("⏳ 等待超时，已自动取消添加。")

    text = resp.extract_plain_text().strip()
    if text.lower() in cancel_words:
        await bocchi_cmd.finish("🛑 已取消添加操作。")
...
async def _(arp: Arparma):
    ...
    if resp is None:
        await bocchi_cmd.finish("⏳ 等待超时，已自动取消修改。")

    text = resp.extract_plain_text().strip()
    if text.lower() in cancel_words:
        await bocchi_cmd.finish("🛑 已取消修改操作。")
...
@bocchi_cmd.assign("prompt.del")
async def _(arp: Arparma):
    persona_name = arp.query("prompt.del.persona_name")
    if not isinstance(persona_name, str):
        await bocchi_cmd.finish("❌ 人设名称非法！")
    if persona_name not in PERSONAS_CACHE:
        await bocchi_cmd.finish(f"❌ 人设 [{persona_name}] 不存在！")
...
async def _(arp: Arparma, session: Uninfo):
    ...
    if not group_id:
        await bocchi_cmd.finish("❌ 私聊环境下请使用 -g 显式指定群组号。")
    ...
    chat_enabled = await group_settings_service.get(
        str(group_id), "bocchi_ai", "random_reply_enable", False
    )
    chat_rate = await group_settings_service.get(
        str(group_id), "bocchi_ai", "random_reply_rate", 0.01
    )
    context_mode = await group_settings_service.get(
        str(group_id), "bocchi_ai", "context_mode", base_config.get("CONTEXT_MODE", "user")
    )

    msg = (
        f"📊 群组 {group_id} 的 bocchi_ai 配置状态：\n"
...
for cmd_alias, key_name in [
    ...
    pconf_cmd.shortcut(
        rf"bocchi设置\s+{cmd_alias}\s+(?P<val>\S+)\s*(?P<targets>.*)",
        command="pconf",
        arguments=["set", f"{key_name}={{val}}", "-p", "bocchi_ai", "{targets}"],
        prefix=True,
    )
