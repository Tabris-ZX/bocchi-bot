...
from bocchi.services.ai.context.memory.compression import (
...
from bocchi.services.ai.capabilities import AbstractCapability, WrapRunHandler
from bocchi.services.ai.context.memory import (
    Isolation,
    MemoryBuilder,
)
from bocchi.services.ai.context.memory.storage import (
    get_orm_chat_context,
)
from bocchi.services.ai.context.rag.backends import TortoiseStorageBackend
from bocchi.services.ai.core.exceptions import AbortException
from bocchi.services.ai.core.messages import ChatResponse, LLMMessage
from bocchi.services.ai.core.templates import PromptTemplate
from bocchi.services.ai.flow.agent import Agent
from bocchi.services.ai.flow.agent.models import AgentConfig
from bocchi.services.ai.flow.base import ConcurrencyPolicy, ConcurrencyScope
from bocchi.services.ai.guardrails import (
    GuardrailAction,
    GuardrailResult,
    output_guardrail,
)
from bocchi.services.ai.llm.api import generate_structured
from bocchi.services.ai.run import RunContext, Inject
from bocchi.services.group_settings_service import group_settings_service
from bocchi.services.log import logger
from bocchi.utils.utils import infer_plugin_namespace
...
from .models import (
    BocchiAiMemoryRecord,
    BocchiAiVectorRecord,
    GroupChatState,
...
bocchi_memory_backend = get_orm_chat_context(BocchiAiMemoryRecord)


async def get_effective_persona_text(group_id: str | None) -> str:
    """辅助方法：统一获取当前生效的人设文本"""
    default_persona = base_config.get("DEFAULT_PERSONA", "波奇")
    persona_name = default_persona
    if group_id:
        persona_name = await group_settings_service.get(
            str(group_id), "bocchi_ai", "current_persona", default_persona
        )
    return (
...
"""
        builder.with_long_term(
            enable=True,
            backend=TortoiseStorageBackend(BocchiAiVectorRecord),
...
def get_chat_model():
    return base_config.get("BOCCHI_AI_CHAT_MODEL")
...
async def persona_guardrail(
    ...
    counts = context.state.setdefault("__bocchi_persona_guardrail_counts__", 0)
    if counts >= max_attempts:
        return GuardrailResult(action=GuardrailAction.PASS)
    context.state["__bocchi_persona_guardrail_counts__"] += 1
...
“你的回复字数超过了15字，并且语气太温柔了，请改用毒舌语气并缩短字数！”）。"""
    ...
    try:
        ...
        logger.warning(f"bocchi_ai 人设护栏裁判执行异常，已放行: {e}", "bocchi_ai")
        return GuardrailResult(action=GuardrailAction.PASS)
...
class TimingGateCapability(AbstractCapability):
    ...
    async def wrap_run(self, context: RunContext, handler: WrapRunHandler) -> Any:
        ...
        is_random_triggered = context.state.get("__bocchi_is_random_triggered__", False)
...
{user_input}
        ...
        try:
            ...
            if not decision.should_reply:
                logger.info(f"Timing Gate 决定静默，原因：{decision.reason}", "bocchi_ai")
                raise AbortException(reason=f"Timing Gate Ignore: {decision.reason}")
        except AbortException:
            raise
        except Exception as e:
            logger.warning(f"Timing Gate 评估异常，放行处理: {e}", "bocchi_ai")

        return await handler()


bocchi_agent = Agent(
    name="bocchi_ai_agent",
...
@bocchi_agent.system_prompt
async def build_bocchi_system_prompt(
    bot: Inject.Bot,
    event: Inject.Event,
    ...
    try:
        ...
        effective_mode = context.state.get("__bocchi_effective_mode__", "user")
        ...
        logger.error(f"构建动态人设 System Prompt 失败: {e}", "bocchi_ai")
            ...
            if config.llm_summary.enable and model_name:
                ...
                if user_turns > config.llm_summary.max_history_turns:
                    if state.is_summarizing:
                        logger.debug("⏳ 正在后台总结压缩中，跳过本次触发...", "bocchi_ai")
                    else:
                        working_msgs = [
                            m
                            for m in state.active_buffer
