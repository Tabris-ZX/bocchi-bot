...
SHUTDOWN_MARK = Path() / "is_shutdown"
...
async def _(bot: Bot, session: Uninfo, flag: str = ArgStr("flag")):
    if flag.lower() in {"true", "是", "好", "确定", "确定是"}:
        ...
        with open(RESTART_MARK, "w", encoding="utf8") as f:
            f.write(f"{bot.self_id} {session.user.id}")
            f.flush()
            os.fsync(f.fileno())
        ...
        if str(platform.system()).lower() == "windows":
            ...
            if not os.access("./tools/restart.sh", os.X_OK):
                os.system("chmod +x ./tools/restart.sh")
...
async def _(bot: Bot, session: Uninfo, flag: str = ArgStr("flag")):
    if flag.lower() in {"1", "是", "好", "确定"}:  # !!! Modified: 支持更多确认词
        ...
        with open(SHUTDOWN_MARK, "w", encoding="utf8") as f:
            f.write(f"{bot.self_id} {session.user.id}")
            f.flush()
            os.fsync(f.fileno())
